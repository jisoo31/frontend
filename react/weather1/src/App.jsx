...
const API_KEY = 'a346cac6a609cff649443955cf29a78f'
...
export default function App() {
  ...
  const axiosWeather = async(cityName) =>{
    ...
    //도시이름 앞뒤에 공백 제거 후 아무것도 입력되지 않으면 도시이름 입력하라는 메세지
    setLoading(true)
    setError('')
    try{
      const response = await axios.get(
        `http://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${API_KEY}&units=metric&leng-kr`
         );
      ...
      <div className="container">
        <h1 className="title">🌤️날씨앱</h1>
          <form className='search-form' onSubmit={handleSubmit}>
            <input type="text" placeholder='도시 이름을 입력하세요(예: Seoul, Tokyo)' 
            value={city}
